...
{
  int i,j,k,l,ii,jj,kk,m,M,flag;
  DataType dtemp,co,x[SIZE_MAX],y[SIZE_MAX];
  DataType temp;
    ...
    for(j=0; j<N; j++)
      {
        ...
        if(flag==0) {
          ...
          y[j]=x[j] + x[ii+jj];
          dtemp=(kk+kk+1)*PI/l;
          y[j+k]=(x[j] - x[ii+jj])/2.0/cos((double)dtemp);
        }
      }
    m=m - 1;
    for(j=0; j<N; j++)
    ...
    for(j=0; j<N; j++)
      {
        ...
        jj=j & (~0 << (m+1));
        flag=ii/2;
        if(ii==flag*2) y[j]=x[jj+flag];
        else if(flag+1 >= k) y[j]=x[jj+flag+k];
        else y[j]=x[jj+flag+k]+x[jj+flag+k+1];
      }
    m=m + 1;
    if(m<M) {
      for(j=0; j<N; j++)
...
void idct_1d(int N, DataType *bin, DataType *bout)
{
    ...
    for(j=0; j<N; j++)
      {
        ...
        ii=j & ~(~0 << (m-1));
        jj=j & (~0 << m);
        if(flag==0) y[j]=x[ii+ii+jj];
        if(flag==1) {
          if(ii==0) y[j]=x[jj+1];
          else
            y[j]=x[ii+ii+jj-1] + x[ii+ii+jj+1];
        }
      }
    m=m - 1;
    for(j=0; j<N; j++)
  ...
  while(m<=M){
    ...
    for(j=0; j<N; j++)
      {
        flag=(j >> (m-1)) & ~(~0 << 1);
        if(flag==0) y[j]=x[j] + x[j+k];
        if(flag==1) {
          jj=(~j) & ~(~0 << m);
          kk=j & (~0 << m);
          y[j]=x[kk+jj] - x[kk+jj+k];
        }
      }
    m=m + 1;
    if(m<=M) {
...
void dct(float *image, int img_dim, int dct_dim, int isgn)
{
  ...
  DataType **blk;
  int i, j, blky, blkx, pby, pbx;

  blk=(DataType **)malloc(dct_dim*dct_dim*sizeof(DataType));
  for(i=0;i<dct_dim;i++)
    blk[i]=(DataType *)malloc(dct_dim*sizeof(DataType));

  for(blky=0; blky<img_dim/dct_dim; blky++) {
    for(blkx=0; blkx<img_dim/dct_dim; blkx++) {
    
      // get one block data
      for(pby=0;pby<dct_dim;pby++)
	for(pbx=0;pbx<dct_dim;pbx++)
	  //blk[pby][pbx]=(DataType) image[blky*dct_dim+pby][blkx*dct_dim+pbx];
	  blk[pby][pbx]=(DataType) image[(blky*dct_dim+pby)*img_dim+(blkx*dct_dim+pbx)];
	...
	dct_2d(isgn, dct_dim, blk);
      
    
      // put it back
      for(pby=0;pby<dct_dim;pby++)
	for(pbx=0;pbx<dct_dim;pbx++)
	  image[(blky*dct_dim+pby)*img_dim+(blkx*dct_dim+pbx)]=(float) blk[pby][pbx];
    }
  }
...
//--- start of 2-D DCT
void dct_2d(int isgn, int blk_dim, DataType **blk_buf)
{

  DataType *bin, *bout;
  int i, j;

  bin=(DataType *) malloc(blk_dim*sizeof(DataType));
  bout=(DataType *) malloc(blk_dim*sizeof(DataType));
  ...
  for(i=0;i<blk_dim;i++){
    ...
    if(isgn==-1)
      dct_1d(blk_dim, bin, bout);
    else
      idct_1d(blk_dim, bin, bout);
